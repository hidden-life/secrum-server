...
func (r *OTPKRepository) BulkInsert(ctx context.Context, keys []*key.OneTimePreKey) error {
	batch := &pgx.Batch{}
	const q = `INSERT INTO one_time_pre_keys (id, device_id, public_key, created_at, used_at) VALUES ($1, $2, $3, $4, $5)`
	for _, k := range keys {
...
FOR UPDATE SKIP LOCKED LIMIT 1`
	...
	// mark as used
	const updQ = `UPDATE one_time_pre_keys SET used_at = NOW() WHERE id = $1`
